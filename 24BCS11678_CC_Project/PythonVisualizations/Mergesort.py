...
'''
    ...
    def mergesortInit(self, code=initMergesortCode, start=True):
        ...
        wait = 0.1
        self.highlightCode('arr = unordered;', callEnviron, wait=wait)
        self.highlightCode('key = identity;', callEnviron, wait=wait)
        self.highlightCode('this->n = n;', callEnviron, wait=wait)
        self.highlightCode('work = new T[n];', callEnviron, wait=wait)
        ...
        self.highlightCode('i = 0; i < n; i++', callEnviron, wait=wait / 2)
        for i in range(self.size):
            self.highlightCode(
                'work[i] = NULL;', callEnviron, wait=wait / 50)
            ...
            self.highlightCode('i = 0; i < n; i++', callEnviron, wait=wait / 20)
        ...
        self.highlightCode('mergesort(0, n);', callEnviron)
    ...
    mergesortCode = '''
void mergesort(int lo={lo}, int hi={hi}) {{
   if (lo + 1 >= hi)
      return;
   int mid = (lo + hi) / 2;
   mergesort(lo, mid);
...
'''
    ...
    def mergesort(self, lo, hi, level=0, code=mergesortCode):
        ...
        self.highlightCode('int mid = (lo + hi) / 2;', callEnviron)
        mid = (lo + hi) // 2
        ...
        self.highlightCode('mergesort(lo, mid);', callEnviron, wait=wait)
        ...
        self.highlightCode('mergesort(mid, hi);', callEnviron, wait=wait)
        delta = (self.CELL_GAP, self.LEVEL_SEPARATION)
        self.moveItemsBy(
            flat(*([self.arrayCells[j]] + [item for item in self.list[j].items]
        ...
        self.highlightCode('merge(lo, mid, hi);', callEnviron)
    ...
    mergeCode = '''
void merge(int lo={lo}, int mid={mid}, int hi={hi}) {{
   int n = 0;
   int idxLo = lo;
   int idxHi = mid;
   while (idxLo < mid && idxHi < hi) {{
      T itemLo = arr[idxLo];
      T itemHi = arr[idxHi];
      if (itemLo <= itemHi) {{
         work[n] = itemLo;
...
'''
    ...
    def merge(self, lo, mid, hi, code=mergeCode):
        ...
        while idxLo < mid and idxHi < hi:
            self.highlightCode(
                'T itemLo = arr[idxLo];', callEnviron, wait=wait)
            self.highlightCode(
                'T itemHi = arr[idxHi];', callEnviron, wait=wait)
            self.highlightCode(
                'itemLo <= itemHi', callEnviron,
                wait=wait)
            if self.list[idxLo].val <= self.list[idxHi].val:
                self.highlightCode('work[n] = itemLo;', callEnviron)
        ...
        while idxLo < mid:
            self.highlightCode(
                'work[n] = arr[idxLo];', callEnviron)
            copiedVal = self.setArrayValue(
                self.workArray, self.workCells, n, 
                self.list, self.arrayCells, idxLo, callEnviron)
            self.workArray[n] = copiedVal

            self.highlightCode(('idxLo++;', 2), callEnviron)
            idxLo += 1
            self.moveItemsBy(idxLoIndex, idxDelta, sleepTime=wait / 10)
            
            self.highlightCode(('n++;', 2), callEnviron)
            n += 1
            self.moveItemsBy(nIndex, idxDelta, sleepTime=wait / 10)
            self.highlightCode(('idxLo < mid', 2), callEnviron, wait=wait)
