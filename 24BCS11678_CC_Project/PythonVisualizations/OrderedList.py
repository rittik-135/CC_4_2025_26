...
"""
    ...
    def newLinkedList(self, code=newLinkedListCode):
        ...
        self.highlightCode('first = nullptr;', callEnviron, wait=wait)
        self.first = None
        self.list = []
        self.display()
    ...
    insertCode = '''
void insert(T newDatum = {val!r}) {{
   T goal = key(newDatum);
   Node* previous = head;
   while (previous->getNext() != nullptr &&
          key(previous->getNext()->getData()) < goal)
      previous = previous->getNext();
   Node* newLink = new Node(newDatum, previous->getNext());
...
'''
    ...
    def insert(self, val, code=insertCode, start=True):
        ...
        previous, column = 0, -1
        self.highlightCode('T goal = key(newDatum);', callEnviron)
        ...
        if self.first:
            self.highlightCode(
                'key(previous->getNext()->getData()) < goal', callEnviron,
                wait=wait)
        ...
        self.highlightCode(
            'Node* newLink = new Node(newDatum, previous->getNext());', callEnviron)
...
bool delete_item(T goal = {goal!r}) {{
   ...
   if (previous->getNext() == nullptr ||
       goal != key(previous->getNext()->getData()))
      throw "No datum with matching key found in list";
...
"""
    ...
    def delete(self, goal, code=deleteCode, start=True, wait=0.1):
        ...
        self.highlightCode('previous->getNext() == nullptr', callEnviron, wait=wait)
        if previous < len(self.list):
            self.highlightCode(
                'goal != key(previous->getNext()->getData())',
                callEnviron, wait=wait)
        ...
        self.highlightCode('Node* toDelete = previous->getNext();', callEnviron, 
                           wait=wait)
...
"""
    ...
    def find(self, goal, code=findCode):
        ...
        if link <= len(self.list):
            self.highlightCode(
                'key(link->getData()) < goal', callEnviron, wait=wait)
