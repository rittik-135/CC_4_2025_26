...
"""
    ...
    def insertionSort(self, code=insertionSortCode, start=True):
        ...
        self.highlightCode('outer = 1; outer < n; outer++', callEnviron)
        while outer < len(self.list):

            # Store item at outer index in temporary mark variable
            self.highlightCode('T temp = a[outer];', callEnviron)
            ...
            self.highlightCode('int inner = outer;', callEnviron)
            inner = outer
            ...
            self.highlightCode('a[inner] = temp;', callEnviron)
...
void bubbleSort() {{
   for (int last = n-1; last > 0; last--) {{
      for (int inner = 0; inner < last; inner++) {{
...
"""
    ...
    def bubbleSort(self, code=bubbleSortCode, start=True):
        ...
        self.highlightCode('last = n-1; last > 0; last--', callEnviron)
        last = n - 1
        ...
        while last > 0:
            ...
            self.highlightCode('inner = 0; inner < last; inner++', callEnviron)
            for inner in range(last):
                ...
                if self.list[inner].val > self.list[inner+1].val:
                    self.highlightCode('swap(inner, inner+1);', callEnviron)
            ...
            last -= 1
            self.highlightCode('last = n-1; last > 0; last--',
                               callEnviron)
...
"""
    ...
    def selectionSort(self, code=selectionSortCode, start=True):
        ...
        # make an index arrow for the outer loop
        self.highlightCode('outer = 0; outer < n-1; outer++', callEnviron)
        outer = 0
        ...
        while outer < len(self.list) - 1:
            ...
            self.highlightCode('int min = outer;', callEnviron)
            min_idx = outer
            if minIndex is None:
            ...
            self.highlightCode('inner = outer+1; inner < n; inner++',
                               callEnviron)
            for inner in range(outer + 1, len(self.list)):
                ...
                self.highlightCode('a[inner] < a[min]',
                                   callEnviron, wait=wait)
            ...
            self.highlightCode('swap(outer, min);', callEnviron)
