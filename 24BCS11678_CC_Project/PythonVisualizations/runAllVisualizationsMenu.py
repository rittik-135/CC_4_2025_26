...
def raiseAppWindowHandler(appWindow, menubutton, menu, backButton=None,
                          adjustForTrinket=False, sticky=(N, E, W, S)):
    def handler():
        ...
        if appWindow != appWindows[0]: # and not adjustForTrinket:
            ...
            if backButton: backButton.grid(row=0, column=0, sticky=W, padx=5)
        else:
            if backButton: backButton.grid_remove()
            menubutton.grid()
    return handler
...
def showVisualizations(   # Display a set of VisualizationApps in pulldown menu
        classes, start=None, title="AlgoViz ⚡ — Data Structures Unleashed", version=None,
    ...
    BG_DARK = '#0d0d1a'
    restoreMenu = Canvas(top, height=MIN_MENUBAR_HEIGHT, bg=BG_DARK)
    restoreMenu.grid(row=0, column=0, sticky=(N, E, W, S))
    menubutton = Menubutton(top, text='Select Visualization', font=MENU_FONT)
    menubutton.grid_remove()  # Hidden on landing; shown when in a visualization

    backButton = Button(top, text='\u2190 Back', font=MENU_FONT,
                        fg='white', bg='#1e1b4b', activeforeground='white',
                        activebackground='#312e81', relief=FLAT, padx=10, bd=0)
    backButton.grid_remove()  # Hidden by default on Intro page
    ...
    top['bg'] = BG_DARK
    intro = Frame(top, bg=BG_DARK)

    # Spacer to push content to vertical center
    Frame(intro, bg=BG_DARK, height=100).pack()

    # App Name
    Label(intro, text='AlgoViz ⚡', font=('Helvetica', 56, 'bold'),
          fg='#7c3aed', bg=BG_DARK).pack(pady=(0, 6))

    # Tagline
    Label(intro, text='Data Structures Unleashed',
          font=('Helvetica', 17), fg='#a78bfa', bg=BG_DARK).pack(pady=(0, 50))

    # Divider line
    Frame(intro, bg='#2d2260', height=1, width=320).pack(pady=(0, 50))

    # Select Visualization button (command wired after menu is created)
    selectBtn = Button(intro, text=' ⚡  Select Visualization  ▾ ',
                       font=('Helvetica', 13, 'bold'),
                       fg='white', bg='#7c3aed',
                       activeforeground='white', activebackground='#5b21b6',
                       relief=FLAT, padx=28, pady=12, cursor='hand2', bd=0)
    selectBtn.pack(pady=6)

    # Loading label (hidden text updated during module loading)
    loading = Label(intro, text='',
                    font=('Helvetica', 9, 'italic'), fg='#4b5563', bg=BG_DARK)
    loading.pack(pady=(30, 0))
    nextline = 4  # Kept for version compat
    ...
    menu.add_command(label='Introduction', 
                     command=raiseAppWindowHandler(intro, menubutton, menu, backButton,
                                                   adjustForTrinket, sticky=()),
                     font=MENU_FONT + chosenStyle)
    menubutton['menu'] = menu
    backButton['command'] = raiseAppWindowHandler(intro, menubutton, menu, backButton)
    # Wire the landing page button to pop up the visualization menu
    selectBtn['command'] = lambda: menu.post(
        selectBtn.winfo_rootx(),
        selectBtn.winfo_rooty() + selectBtn.winfo_height())
    ...
    for folder in folders:
        ...
        for app in folders[folder]:
            ...
            menu.add_command(
                ...
                command=raiseAppWindowHandler(pane, menubutton, menu, backButton,
                                              adjustForTrinket))

    if startAppWindow and startAppWindow != appWindows[0]:
        backButton.grid(row=0, column=0, sticky=W, padx=5)
    ...
    if version:
        if isinstance(version, str):  # Hidden handler to show version info
            ...
            spacer = Label(intro, text=' ', font=INTRO_FONT, bg=BG_DARK)
            version = Label(intro, text='version {}.{}'.format(*version),
                            font=(INTRO_FONT[0], 2 - abs(INTRO_FONT[1])),
                            fg='#6b7280', bg=BG_DARK)
            spacer.pack()
            version.pack()
        else:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '-t', '--title',  default='AlgoViz ⚡ — Data Structures Unleashed',
