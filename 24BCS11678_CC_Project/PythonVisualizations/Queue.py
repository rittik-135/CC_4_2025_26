...
'''
    ...
    def isEmpty(self, code=isEmptyCode):
        ...
        self.highlightCode('return nItems == 0;', callEnviron, wait=0.1)
        self.cleanUp(callEnviron)
        return self.nItems == 0

    isFullCode = '''
bool isFull() {{ return nItems == maxSize; }}
'''

    def isFull(self, code=isFullCode):
        callEnviron = self.createCallEnvironment(code=code)
        self.highlightCode('return nItems == maxSize;',
                           callEnviron, wait=0.1)
        self.cleanUp(callEnviron)
        return self.nItems >= self.size
    
    insertCode = '''
bool insert(T item = {val!r}) {{
   if (isFull())
...
'''
    ...
    def insertRear(self, val, code=insertCode, start=True):
        ...
        self.highlightCode('isFull()', callEnviron, wait=wait)
        if self.isFull():
            self.highlightCode(
                'throw "Queue overflow";', callEnviron, wait=wait,
                color=self.EXCEPTION_HIGHLIGHT)
            self.cleanUp(callEnviron)
            return False

        # increment rear
        self.highlightCode('rear++;', callEnviron, wait=wait)
        ...
        self.highlightCode(
            'rear == maxSize', callEnviron, wait=wait)
        if nextRear == 0:
            self.highlightCode('rear = 0;', callEnviron, wait=wait)

        # insert the item
        self.highlightCode(
            'que[rear] = item;', callEnviron, wait=wait)
...
'''
    ...
    def removeFront(self, code=removeFrontCode, start=True):
        ...
        callEnviron |= set(box)
        self.highlightCode(
            'T front_val = que[front];', callEnviron, wait=wait)
        ...
        self.highlightCode('que[front] = NULL;', callEnviron)
        quadrant = [E, N, W, S][
        ...
        #increment front
        self.highlightCode('front++;', callEnviron)
        newFront = (self.front + 1) % self.size
        if self.frontArrow:
            self.moveIndexTo(self.frontArrow, newFront, self.frontLevel)
        self.highlightCode(
            'front == maxSize', callEnviron, wait=wait)
        if newFront == 0:
            self.highlightCode('front = 0;', callEnviron, wait=wait)
        self.front = newFront
        
        # decrement number of items
        self.highlightCode('nItems--;', callEnviron, wait=wait)
        self.updateNItems(self.nItems - 1)

        self.highlightCode('return front_val;', callEnviron, wait=wait)
        self.cleanUp(callEnviron)
        return n.val
...
'''
    ...
    def peek(self, code=peekCode, start=True):
        ...
        self.highlightCode('isEmpty()', callEnviron, wait=wait)
        if self.isEmpty():
            self.highlightCode('return NULL;', callEnviron, wait=wait)
            self.cleanUp(callEnviron)
            return None

        # create the output box
        self.highlightCode('return que[front];', callEnviron)
    ...
    def display(self, newSize=None, code=newCode, start=True):
        ...
        if callEnviron:
            ...
            try:
                ...
                self.highlightCode(
                    'maxSize = size;', callEnviron, wait=wait)
                self.highlightCode(
                    'que = new T[size];', callEnviron, wait=wait)
            except UserStop:
                wait = 0
            self.size = newSize
        ...
        if callEnviron and wait:
            try:
                self.highlightCode('front = 1;', callEnviron, wait=wait)
            except UserStop:
                wait = 0
        ...
        if callEnviron and wait:
            try:
                self.highlightCode('nItems = 0;', callEnviron, wait=wait)
            except UserStop:
                wait = 0
