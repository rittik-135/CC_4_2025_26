...
'''
    ...
    def new(self, val, code=newCode, start=True):
        ...
        self.size = val
        self.highlightCode('a = new T[initialSize];', callEnviron, 
                           wait=0.1)
        self.list = []
        ...
        self.highlightCode('nItems = 0;', callEnviron, wait=0.1)
        self.display()
        self.wait(0.1)
...
"""
    ...
    def insert(self, val, allowGrowth=False, code=insertCode, start=True):
        ...
        self.highlightCode('a[nItems] = item;', callEnviron)
...
"""
    ...
    def get(self, n, code=getCode):
        ...
        result = None
        if 0 <= n:
            self.highlightCode('n < nItems', callEnviron, wait=wait)
            if n < len(self.list):
                self.highlightCode('return a[n];', callEnviron, wait=wait)
                result = self.list[n]
            else:
                self.highlightCode([], callEnviron)
        else:
...
"""
    ...
    def search(self, item, code=searchCode, start=True):
        callEnviron = self.createCallEnvironment(
            code=code.format(**locals()), startAnimations=start)
        self.highlightCode('find(item)', callEnviron)
        n = self.find(item)
        callEnviron |= set(self.createIndex(n, '', level=2))
        if n > -1:
            callEnviron.add(self.createFoundCircle(n))
        self.highlightCode('get(find(item))', callEnviron)
        result = self.get(n)
        self.highlightCode('return get(find(item));', callEnviron)
        self.wait(0.2)
        self.cleanUp(callEnviron)
        return result
...
"""
    ...
    def delete(self, val, code=deleteCode, start=True):
        ...
        # show that we are starting the loop
        self.highlightCode('j = 0; j < nItems; j++', callEnviron, wait=0.1)
...
'''
    ...
    def deleteLast(self, code=deleteLastCode, start=True):
        ...
        self.highlightCode('a[nItems] = NULL;', callEnviron)
...
"""
    ...
    def traverse(self, code=traverseCode, start=True):
        ...
        for j, n in enumerate(self.list):
            ...
            self.highlightCode('print(a[j]);', callEnviron)
