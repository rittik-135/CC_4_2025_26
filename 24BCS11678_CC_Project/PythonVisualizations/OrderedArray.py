...
class OrderedArray(SortingBase):
    ...
    insertCode = '''
void insert(T item = {val}) {{
   if (nItems >= size)
      throw "Array overflow";

   int index = find(item);
   for (int j = nItems; j > index; j--)
      a[j] = a[j-1];
...
'''
    ...
    def insert(self, val, code=insertCode, start=True):
        ...
        self.highlightCode('nItems >= size', 
                           callEnviron, wait=wait)
        if len(self.list) >= self.size:
            self.highlightCode(
                'throw "Array overflow"', callEnviron, wait=wait * 2,
                color=self.EXCEPTION_HIGHLIGHT)
            self.cleanUp(callEnviron)
            return

        self.highlightCode('int index = find(item);', callEnviron)
        index = self.find(val)
        indexIndex = self.createIndex(index, 'index', level=2)
        callEnviron |= set(indexIndex)
        
        self.highlightCode('int j = nItems; j > index; j--', callEnviron,
                           wait=wait)
        ...
        for j in range(len(self.list), index, -1):
            ...
            self.highlightCode('a[j] = a[j-1];', callEnviron)
            self.assignElement(j - 1, j, callEnviron, sleepTime=wait / 10)
            
            self.highlightCode('int j = nItems; j > index; j--', 
                               callEnviron)
        ...
        self.highlightCode('a[index] = item;', callEnviron, wait=wait)
...
'''
    ...
    def find(self, val, code=findCode):
        ...
        self.highlightCode('hi = nItems-1;', callEnviron)
        hi = len(self.list) - 1
        ...
        while lo <= hi:
            ...
            self.highlightCode('a[mid] == item', callEnviron, wait=wait)
    ...
    searchCode = """
T* search(T item = {item}) {{
   int index = find(item);
   if (index < nItems && a[index] == item)
      return a[index];
}}
"""
    ...
    def search(self, item, code=searchCode, start=True):
        ...
        self.highlightCode('find(item)', callEnviron, wait=wait)
        index = self.find(item)
        callEnviron |= set(self.createIndex(index, 'index'))

        result = None
        self.highlightCode('index < nItems', callEnviron, wait=wait)
        if index < len(self.list):
            self.highlightCode('a[index] == item', callEnviron, wait=wait)
            if self.list[index].val == item:
                callEnviron.add(self.createFoundCircle(index))
                self.highlightCode('return a[index];', callEnviron, wait=wait)
...
'''
    ...
    def delete(self, val, code=deleteCode, start=True):
        ...
        self.highlightCode('j < nItems', callEnviron, wait=wait)

        if j < len(self.list):
            self.highlightCode('a[j] == item', callEnviron, wait=wait)
