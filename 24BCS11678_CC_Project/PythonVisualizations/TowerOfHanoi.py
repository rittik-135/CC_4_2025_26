...
class TowerOfHanoi(VisualizationApp):
    ...
    _init__Code = '''
TowerOfHanoi(int nDisks={nDisks}) {{
   stacks = new Stack[3];
   labels[] = {{"L", "M", "R"}};
   this->nDisks = nDisks;
   reset();
...
void reset() {{
   for (int spindle = 0; spindle < 3; spindle++) {{
      stacks[spindle] = new Stack(nDisks);
      if (spindle == 0)
         for (int disk = nDisks; disk > 0; disk--)
            stacks[spindle]->push(disk);
   }}
}}
'''
    ...
    def setupDisks(self, nDisks, code=_init__Code, start=True):
        ...
        if callEnviron:
            self.highlightCode('stacks = new Stack[3];', callEnviron, wait)
            self.highlightCode('labels[] = {"L", "M", "R"};',
                               callEnviron, wait)
            spindleLabels = [
                self.createSpindleLabel(label, spindle) 
                for spindle, label in enumerate(['L', 'M', 'R'])]
            callEnviron |= set(spindleLabels)
            self.highlightCode('this->nDisks = nDisks;', callEnviron, wait)
            self.highlightCode('reset();', callEnviron, wait)
            ...
            self.highlightCode('spindle = 0; spindle < 3; spindle++', callEnviron2)
        ...
        for spindle in range(3):
            ...
            if spindle == 0: # On left spindle
                if callEnviron:
                    self.highlightCode(
                        'disk = nDisks; disk > 0; disk--',
                        callEnviron2, wait)
                for ID in range(nDisks - 1, -1, -1):
                    if callEnviron:
                        self.highlightCode('stacks[spindle]->push(disk);',
                                           callEnviron2, wait)
    ...
    solveCode = '''
void solve(int nDisks={nDisks}, int start={start}, int goal={goal}, int spare={spare}) {{
   if (nDisks <= 0) return;
   solve(nDisks - 1, start, spare, goal);
   move(start, goal);
   solve(nDisks - 1, spare, goal, start);
}}
'''
    ...
    def solve(self, nDisks=None, start=0, goal=2, spare=1, 
        ...
        self.highlightCode('solve(nDisks - 1, start, spare, goal);',
                           callEnviron)
        self.solve(nDisks - 1, start, spare, goal)
        self.moveItemsTo(labelItems, toCoords, sleepTime=moveWait)
        self.canvas.itemconfigure(labelItems[0], text=startLabel)
        self.highlightCode('move(start, goal);', callEnviron)
        self.moveDisk(start, goal)
        self.moves += 1
        self.highlightCode('solve(nDisks - 1, spare, goal, start);',
                           callEnviron)
