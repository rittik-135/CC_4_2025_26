...
'''
    ...
    def medianOfThree(self, lo, hi, code=medianOfThreeCode):
        ...
        self.highlightCode('int mid = (lo + hi) / 2;', callEnviron, wait=wait)
        mid = (lo + hi) // 2
        midIndex = self.createIndex(mid, 'mid', level=2)
        callEnviron |= set(midIndex)

        self.highlightCode('get(lo) > get(mid)',
                           callEnviron, wait=wait)
        if self.list[lo].val > self.list[mid].val:
            self.highlightCode('swap(lo, mid);', callEnviron)
            self.swap(lo, mid)

        self.highlightCode('get(lo) > get(hi)',
                           callEnviron, wait=wait)
        if self.list[lo].val > self.list[hi].val:
            self.highlightCode('swap(lo, hi);', callEnviron)
            self.swap(lo, hi)

        self.highlightCode('get(hi) > get(mid)',
                           callEnviron, wait=wait)
        if self.list[hi].val > self.list[mid].val:
            self.highlightCode('swap(hi, mid);', callEnviron)
            self.swap(hi, mid)

        self.highlightCode('return get(hi);', callEnviron, wait=wait)
        self.cleanUp(callEnviron, sleepTime=codeWait)
        return self.list[hi].val

    __partCode = '''
int partition(T pivot={pivot}, int lo={lo}, int hi={hi}) {{
   while (lo <= hi) {{
      while (get(lo) < pivot)
         lo++;
      while (pivot < get(hi))
...
'''
    def partition(self, pivot, lo, hi, code=__partCode):
        ...
        while lo <= hi:
            self.highlightCode('get(lo) < pivot', callEnviron,
                               wait=wait)
            while (len(code) > 0 or lo <= hi) and self.list[lo].val < pivot:
                self.highlightCode('lo++;', callEnviron)
                lo += 1
            ...
            if lo < hi or self.useMedianOf3.get():
                self.highlightCode('pivot < get(hi)', callEnviron,
                                   wait=wait)
            while (self.useMedianOf3.get() or lo < hi) and (
                    pivot < self.list[hi].val):
                self.highlightCode('hi--;', callEnviron)
                hi -= 1
            ...
            self.highlightCode('lo++; hi--;', callEnviron)
            lo, hi = lo + 1, hi - 1
    ...
    quicksortCode = '''
void quicksort(int lo={lo}, int hi={hi}, int shortSize=3) {{
   if (hi < 0)
      hi = size - 1;
   shortSize = max(3, shortSize);
   if (hi - lo + 1 <= shortSize)
      return insertionSort(lo, hi);
   T pivotItem = medianOfThree(lo, hi);
   int hipart = partition(pivotItem, lo + 1, hi - 1);
   swap(hipart, hi);
   quicksort(lo, hipart - 1, shortSize);
   quicksort(hipart + 1, hi, shortSize);
}}
'''
    ...
    def quicksort(
        ...
        self.highlightCode('hi < 0', callEnviron, wait=wait)
        if hi is None:
            self.highlightCode('hi = size - 1;', callEnviron, wait=wait)
            hi = len(self.list) - 1
            hiIndex = self.createIndex(hi, 'hi', level=2)
            callEnviron |= set(hiIndex)

        self.highlightCode('shortSize = max(3, shortSize);', callEnviron, wait=wait)
        short = max(3, short)

        self.highlightCode('hi - lo + 1 <= shortSize', callEnviron, wait=wait)
        if hi - lo + 1 <= short:
            self.highlightCode('return insertionSort(lo, hi);',
                               callEnviron, wait=wait)
        ...
        self.highlightCode('T pivotItem = {}(lo, hi);'.format(
        ...
        self.highlightCode(
            'int hipart = partition(pivotItem, {}, hi - 1);'.format(
        ...
        self.highlightCode('swap(hipart, hi);', callEnviron)
        self.swap(hipart, hi)
        ...
        self.highlightCode('quicksort(lo, hipart - 1, shortSize);',
                           callEnviron)
        ...
        self.highlightCode('quicksort(hipart + 1, hi, shortSize);',
                           callEnviron)
        colors = self.canvas.fadeItems(loIndex + hiIndex + hipartIndex)
...
int shellSort() {{
    ...
    while (h > 0) {{
        for (int outer = h; outer < size; outer++) {{
            T temp = get(outer);
            int inner = outer;
            while (inner >= h && temp < get(inner-h)) {{
                set(inner, get(inner-h));
                inner -= h;
                nShifts++;
            }}
            if (inner < outer) {{
    ...
    def shellSort(self, code=shellSortCode, start=True):
        ...
        # calculate h
        self.highlightCode('int h = 1;', callEnviron, wait=wait)
        h = 1
        hLabel = None
        hLabel = self.adjustHLabel(h, 0, None)
        callEnviron |= set(hLabel)
        
        self.highlightCode('3 * h + 1 < size', callEnviron, wait=wait)
        ...
        self.highlightCode('int nShifts = 0;', callEnviron, wait=wait)
        nShifts = 0
        ...
        while h > 0:
            ...
            self.highlightCode('int outer = h; outer < size; outer++', callEnviron,
                               wait=wait)
            for outer in range(h, len(self.list)):
                ...
                self.highlightCode('T temp = get(outer);', callEnviron)
                ...
                self.highlightCode('int inner = outer;', callEnviron, wait=wait)
                inner = outer
                ...
                if inner >= h:
                    self.highlightCode('temp < get(inner-h)', callEnviron,
                                       wait=wait)
                    
                while inner >= h and tempVal.val < self.list[inner-h].val:
                    self.highlightCode('set(inner, get(inner-h));',
                                       callEnviron)
                    self.assignElement(
                    ...
                    self.highlightCode('nShifts++;', callEnviron, wait=wait)
                    nShifts += 1
                    ...
                    self.highlightCode('inner >= h', callEnviron, wait=wait)
                    if inner >= h:
                        self.highlightCode('temp < get(inner-h)',
                                           callEnviron, wait=wait)

                self.highlightCode('inner < outer', callEnviron, wait=wait)
                if inner < outer:
                    self.highlightCode('set(inner, temp);', callEnviron)
                    ...
                    self.highlightCode(('nShifts++;', 2), callEnviron,
                                       wait=wait)
                    nShifts += 1
                    self.canvas.itemConfig(
                        nShiftsLabel, text=nShiftsValue.format(nShifts))
                ...
                self.highlightCode('int outer = h; outer < size; outer++',
                                   callEnviron, wait=wait)
