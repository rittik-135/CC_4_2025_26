...
T first() {{
   if (isEmpty())
      throw "No first item in empty list";
   return head->getData();
}}
"""
    ...
    def firstData(self, code=firstCode, animateOutput=True, start=True):
        ...
        if self.first is None:
            self.highlightCode(
                'throw "No first item in empty list";', callEnviron,
                wait=wait, color=self.EXCEPTION_HIGHLIGHT)
        else:
            self.highlightCode(
                'return head->getData();', callEnviron)
            if animateOutput:
...
T deleteFirst() {{
   if (isEmpty())
      throw "Cannot delete from empty linked list";
...
"""
    ...
    def deleteFirst(self, code=deleteFirstCode, start=True):
        ...
        self.highlightCode('Node* first = head;', callEnviron, wait=wait)
        firstIndex = self.createIndex(1, 'first')
        callEnviron |= set(firstIndex)
        
        self.highlightCode('head = first->getNext();', callEnviron,
                           wait=wait)
        previous = 0
...
"""
    ...
    def traverse(self, code=traverseCode, start=True):
        ...
        self.highlightCode('Node* link = head;', callEnviron)
        link = 1
        linkIndex = self.createIndex(link, name='link')
        callEnviron |= set(linkIndex)
        self.wait(wait)
        
        self.highlightCode('link != nullptr', callEnviron, wait=wait)
        while link <= len(self.list):
            self.highlightCode('print(link->getData());', callEnviron)
...
"""
    ...
    def newLinkedList(self, code=newLinkedListCode):
        ...
        self.highlightCode('first = nullptr;', callEnviron)
        self.first = None
        self.list = []
        self.display()
    ...
    insertCode = '''
void insert(T datum = {val!r}) {{
   Node* link = new Node(datum, head);
   head = link;
}}
'''
    ...
    def insert(self, val, code=insertCode, start=True, wait=0.1):
        ...
        self.highlightCode('Node* link = new Node(datum, head);', callEnviron)
        ...
        self.highlightCode('head = link;', callEnviron)
        previous = 0
    ...
    deleteCode = """
bool delete_item(T goal = {goal!r}) {{
   if (isEmpty())
      throw "Cannot delete from empty linked list";

   Node* previous = head;
   while (previous->getNext() != nullptr) {{
      Node* link = previous->getNext();
      if (goal == link->getData()) {{
   ...
   throw "No item with matching key found in list";
}}
"""
    ...
    def delete(self, goal, code=deleteCode, start=True, wait=0.1):
        ...
        self.highlightCode('Node* previous = head;', callEnviron)
        previous = 0
        ...
        self.highlightCode(
            'previous->getNext() != nullptr', callEnviron, wait=wait)
        while previous < len(self.list):
            
            self.highlightCode('Node* link = previous->getNext();', callEnviron)
            link = previous + 1
            if linkIndex:
                self.moveItemsTo(
                    linkIndex, 
            ...
            self.highlightCode(
                'goal == link->getData()', callEnviron, wait=wait)
    ...
    findCode = """
Node* find(T goal = {goal!r}) {{
   Node* link = head;
   while (link != nullptr) {{
      if (link->getData() == goal)
         return link;
      link = link->getNext();
   }}
}}
"""
    ...
    def find(self, goal, code=findCode):
        ...
        while link <= len(self.list):
            self.highlightCode('link->getData() == goal', callEnviron,
                               wait=wait)
...
"""
    ...
    def search(self, goal, code=searchCode, start=True):
        ...
        self.highlightCode('Node* link = find(goal);', callEnviron)
        link = self.find(goal)
